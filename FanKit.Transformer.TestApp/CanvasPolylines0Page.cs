    ...
    public sealed partial class CanvasPolylines0Page : CanvasSolo0Page
    {
        ...
        public override void Draw(ICanvasResourceCreator resourceCreator, CanvasDrawingSession drawingSession)
        {
            foreach (Layer item in this.Layers)
            {
                ...
                {
                    drawingSession.DrawDashActualPolyline(figure.Data, figure.IsClosed, item.ActualStrokeWidth);
                }
            }
        ...
        private bool ContainsPoint(Layer item, Vector2 point)
        {
            ...
            for (int i = 0; i < item.Data.Count; i++)
            {
                ...
                this.FootPoint = new Polylines.FootPointer(NodePointUnits.Actual, figure.Data, figure.IsClosed, point, ls);
                    ...
                    for (int k = 0; k < this.Layers.Count; k++)
                    {
                        ...
                        if (item.IsSelected)
                        {
                            for (int j = 0; j < item.Data.Count; j++)
                            {
                                ...
                                this.Inserter = new SegmentInserter(ref this.FootPoint, NodePointUnits.Normal, figure.Data, figure.IsClosed, this.Position, ls);
                        ...
                        for (int k = 0; k < this.Layers.Count; k++)
                        {
                            ...
                            if (item.IsSelected)
                            {
                                for (int j = 0; j < item.Data.Count; j++)
                                {
                                    ...
                                    this.FootPoint = new FootPointer(NodePointUnits.Normal, figure.Data, figure.IsClosed, this.Position, ls);
                                    if (this.FootPoint.Contains)
                                    {
                                        this.Invalidate(InvalidateModes.CanvasControl);
                                        return;
