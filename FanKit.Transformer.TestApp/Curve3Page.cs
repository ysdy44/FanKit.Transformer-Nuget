...
using Layer = FanKit.Transformer.Demos.DemoCurve0;
...
namespace FanKit.Transformer.TestApp
{
    ...
    /// </summary>
    public sealed partial class Curve3Page : SoloPage
    {
        //@Key
        ...
        public override void Draw(ICanvasResourceCreator resourceCreator, CanvasDrawingSession drawingSession)
        {
            ...
            {
                path.CreatePath(this.Layer.Segments, this.Layer.IsClosed);
        ...
        private void Invalidate(InvalidateModes modes)
        {
            ...
            if (modes.HasFlag(InvalidateModes.InitIndicator))
            {
                ...
                {
                    this.Composer.UnionByPoint(item);
                }
        ...
        private void Draw1(CanvasDrawingSession drawingSession)
        {
            foreach (Segment segment in this.Layer.Segments)
            {
                ...
                {
                    drawingSession.DrawLine(segment.Point.Point, segment.Point.LeftControlPoint, Windows.UI.Colors.DeepSkyBlue);
                    drawingSession.DrawLine(segment.Point.Point, segment.Point.RightControlPoint, Windows.UI.Colors.DeepSkyBlue);
                }
            }
            ...
            foreach (Segment segment in this.Layer.Segments)
            {
                if (segment.IsChecked)
                    ...
                    drawingSession.DrawNode3(segment.Point.Point);
            }
        }
        ...
        private void Draw3(CanvasDrawingSession drawingSession)
        {
            ...
            {
                if (segment.IsChecked)
                    drawingSession.DrawNode(segment.Point.Point);
                else
                    drawingSession.DrawNode3(segment.Point.Point);
            }
        ...
        private void CacheSingle2()
        {
            ...
            switch (this.Indexer.Mode)
            {
                case SegmentIndexerMode.None:
                    ...
                    this.Inserter = new SegmentInserter(out this.Closest, this.Layer.Segments, this.Layer.IsClosed, this.Point, ls);
                ...
                case SegmentIndexerMode.PointWithoutChecked:
                    {
                        ...
                        this.Composer.ResetByPoint(segment.Point.Point);
        ...
        private void Over2()
        {
            ...
            switch (this.Indexer.Mode)
            {
                case SegmentIndexerMode.None:
                    ...
                    {
                        this.Closest = new ClosestPointer(this.Layer.Segments, this.Layer.IsClosed, this.Point, ls);
                        if (this.Closest.Contains)
                        {
                            this.Invalidate(InvalidateModes.CanvasControl);
                            return;
