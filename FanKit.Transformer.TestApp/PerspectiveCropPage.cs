...
namespace FanKit.Transformer.TestApp
{
    ...
    public sealed partial class PerspectiveCropPage : DualPage
    {
        ...
        public override void DrawSource(ICanvasResourceCreator resourceCreator, CanvasDrawingSession drawingSession)
        {
            if (this.Bitmap != null)
            {
                drawingSession.DrawImage(this.Bitmap);
            }
        ...
        private void Invalidate(InvalidateModes modes)
        {
            if (modes.HasFlag(InvalidateModes.InitIndicator))
            {
                ...
                this.FreeTransformer.UpdateCanvas();
            }
        ...
        private void Single0()
        {
            switch (this.Mode)
            {
                ...
                default:
                    if (this.KeepConvex)
                        this.FreeTransformer.MovePointOfConvexQuadrilateral(this.Point);
                    else
