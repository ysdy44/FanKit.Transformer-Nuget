    ...
    public sealed partial class Curves0Page : SoloPage
    {
        ...
        public override void Draw(ICanvasResourceCreator resourceCreator, CanvasDrawingSession drawingSession)
        {
            foreach (Layer item in this.Layers)
            {
                ...
                {
                    path.CreateMapPath(item.Figures);
                        ...
                        if (item.IsSelected)
                        {
                            for (int j = 0; j < item.Figures.Count; j++)
                            {
                                ...
                                if (this.Indexer.Mode != NodeIndexerMode.None)
                                {
                                    ...
                                    if (this.Indexer.Mode == NodeIndexerMode.LeftControlPoint) goto case NodeIndexerMode.LeftControlPoint;
                                    if (this.Indexer.Mode == NodeIndexerMode.RightControlPoint) goto case NodeIndexerMode.RightControlPoint;
                                }
                            }
                        }
                    }
                    ...
                    for (int k = 0; k < this.Layers.Count; k++)
                    {
                        ...
                        if (item.IsSelected)
                        {
                            for (int j = 0; j < item.Figures.Count; j++)
                            {
                                ...
                                this.Inserter = new SegmentInserter(ref this.Closest, figure.Segments, figure.IsClosed, this.Point, ls);
