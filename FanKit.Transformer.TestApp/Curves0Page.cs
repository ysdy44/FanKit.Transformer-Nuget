    ...
    public sealed partial class Curves0Page : SoloPage
    {
        ...
        private bool ContainsPoint(Layer item, Vector2 point)
        {
            ...
            for (int i = 0; i < item.Data.Count; i++)
            {
                ...
                this.Closest = new Curves.ClosestPointer(figure.Data, figure.IsClosed, point, ls);
            ...
            switch (this.Indexer.Mode)
            {
                        ...
                        if (item.IsSelected)
                        {
                            for (int j = 0; j < item.Data.Count; j++)
                            {
                                ...
                                if (this.Indexer.Mode != SegmentMode.None)
                                {
                                    ...
                                    if (this.Indexer.Mode == SegmentMode.LeftControlPoint) goto case SegmentMode.LeftControlPoint;
                                    if (this.Indexer.Mode == SegmentMode.RightControlPoint) goto case SegmentMode.RightControlPoint;
                                }
                            }
                        }
                    }
                    ...
                    for (int k = 0; k < this.Layers.Count; k++)
                    {
                        ...
                        if (item.IsSelected)
                        {
                            for (int j = 0; j < item.Data.Count; j++)
                            {
                                ...
                                this.Inserter = new SegmentInserter(ref this.Closest, figure.Data, figure.IsClosed, this.Point, ls);
                ...
                case SegmentMode.PointWithoutChecked:
                    for (int k = 0; k < this.Layers.Count; k++)
                    {
                        ...
                        {
                            if (this.Index1 == k)
                                item.Select(this.Index2, this.Indexer.Index);
                            else
                                item.DeselectAll();
                        }
                    }
                ...
                case SegmentMode.RightControlPoint:
                    {
                        ...
                        bool isLeft = this.Indexer.Mode == SegmentMode.LeftControlPoint;
                        ...
                        for (int k = 0; k < this.Layers.Count; k++)
                        {
                            ...
                            if (item.IsSelected)
                            {
                                for (int j = 0; j < item.Data.Count; j++)
                                {
                                    ...
                                    this.Closest = new ClosestPointer(figure.Data, figure.IsClosed, this.Point, ls);
                                    if (this.Closest.Contains)
                                    {
                                        this.Invalidate(InvalidateModes.CanvasControl);
                                        return;
