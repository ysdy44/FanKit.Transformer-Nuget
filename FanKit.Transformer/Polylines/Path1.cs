...
namespace FanKit.Transformer.Polylines
{
    public partial class Path1 : IGetFigure
    {
        ...
        public Path1(List<Segment1> items, Matrix3x2 matrix)
        {
            this.Data = items;
            //this.Matrix = matrix;
            //this.Invert();
        ...
        {
            this.BeginExtend();
            foreach (Segment1 item in this.Data)
            {
                this.Extend(item.Point);
            }
            this.EndExtend();
        ...
        private void TranslateRaw()
        {
            ...
            for (int i = 0; i < this.Data.Count; i++)
            {
                ...
                if (item.IsChecked)
                {
                    Vector2 p = new Vector2(item.Point.X + this.HostTranslateX, item.Point.Y + this.HostTranslateY);
                    this.Data[i] = new Segment1
                    {
                        IsChecked = true,
                        Point = p,
        ...
        private void TransformMap()
        {
            ...
            for (int i = 0; i < this.Data.Count; i++)
            {
                ...
                if (item.IsChecked)
                {
                    Vector2 p = Vector2.Transform(item.Point, this.HostMatrix);
                    this.Data[i] = new Segment1
                    {
                        IsChecked = true,
                        Point = p,
